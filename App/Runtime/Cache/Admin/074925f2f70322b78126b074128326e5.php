<?php if (!defined('THINK_PATH')) exit();?><div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<?php echo U(ACTION_NAME);?>" method="post">
        ...
        <div class="bjui-searchBar">
            <label>共有<?php echo ($files); ?>个SQL备份文件，共计<?php echo ($total); ?></label>
        ...
        <table data-toggle="tablefixed" data-width="100%" data-nowrap="true">
            ...
            <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sql): $mod = ($i % 2 );++$i;?><tr>
                        <td>
                            <input pre="<?php echo ($sql["pre"]); ?>" type="checkbox" name="sqlFiles[]" value="<?php echo ($sql["name"]); ?>" />
                        </td>
...
<script type="text/javascript">
    $(function() {
        ...
        $(".restore").click(function() {
            ...
            $(".btn").attr("disabledSubmit", true);
            $.getJSON("/index.php/Admin/SysData/restoreData", {
                sqlPre: sqlPre
            }, function(json) {
                if (json.status == 1) {
                    if (json.url) {
